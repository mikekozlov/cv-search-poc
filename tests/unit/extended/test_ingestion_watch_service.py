...
def test_candidate_id_uses_candidate_folder() -> None:
    cid1 = candidate_id_from_source_gdrive_path("Engineering/backend/CV.pptx")
    cid2 = candidate_id_from_source_gdrive_path("Engineering/backend/Resume.pptx")
    cid3 = candidate_id_from_source_gdrive_path("Sales/backend/CV.pptx")
    assert cid1 == cid2
    assert cid1 != cid3
...
def test_reconcile_once_picks_latest_non_archived_file(tmp_path) -> None:
    inbox_dir = tmp_path / "inbox"
    candidate_dir = inbox_dir / "CANDIDATES" / "NET" / "Jane Doe"
    candidate_dir.mkdir(parents=True, exist_ok=True)
    old_path = candidate_dir / "Jane Doe CV (OLD).pptx"
    new_path = candidate_dir / "Jane Doe CV.pptx"
    old_path.write_text("old", encoding="utf-8")
    new_path.write_text("new", encoding="utf-8")
    old_mtime = time.time() + 5
    os.utime(old_path, (old_mtime, old_mtime))

    rel_new = new_path.relative_to(inbox_dir).as_posix()
    redis_client = InMemoryRedisClient()
    db = _StubDB(last_updated_by_path={})
    svc = FileWatchService(
        inbox_dir=inbox_dir,
        redis=redis_client,
        db=db,
    ...
    payload = redis_client.pop_from_queue("ingest:queue:extract:test", timeout=1)
    assert payload
    assert payload["source_gdrive_path"] == rel_new
